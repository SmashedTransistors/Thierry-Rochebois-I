...
public class TRIEditor extends JPanel implements Receiver {
    ...
    public void send(MidiMessage message, long timeStamp) {
        ...
        switch(control){
            case 74: {getCurrentPreset().fCut=value; }  break;
            case 73: {getCurrentPreset().fMod=value; }  break;
            case 72: {getCurrentPreset().fEnv=value; }  break;
            case 16: {getCurrentPreset().vEnv=value; }  break;
            case 1:  {getCurrentPreset().mrph=value; }  break;
            case 18: {getCurrentPreset().mMod=value; }  break;
            case 19: {getCurrentPreset().mRat=value; } break;

            case 91: {getCurrentPreset().rev=value<=7?value:7;  }   break;
            case 93: {getCurrentPreset().ch=value<=3?value:3;    }         break;
            case 94: {getCurrentPreset().sym=value<=7?value:7;   }        break;
            case 71: {getCurrentPreset().fRes=value<=5?value:5; ;    }      break;
            case 78: {getCurrentPreset().a1=value<32?value:31;   }              break;
            case 79: {getCurrentPreset().b1=value<33?value:32;   }              break;
            case 75: {getCurrentPreset().a0=value<32?value:31;   }              break;
            case 76: {getCurrentPreset().b0=value<33?value:32;   }              break;
            case 77: {
    ...
    JButton bSaveToTeensy =new JButton("Save to Preset ");
    ...
    private void initPresets(){
        ...
        presets[7]=new Preset("phaser",91,64-64,  76,   2, 110,   0,  14,  25,  8, 8,    4, 6, 6,  0,  5, 1);
        ...
        presets[11]=new Preset("space",100,64-64, 121,   4,  51,   0,  14,  93, 14,15,    4,12,12,  3,  5, 3);
        presets[12]=new Preset("harsh1",100,64-13,  73,   0,  14,  75,  74,  60, 17,10,    2,16, 4,  0,  3, 0);
        presets[13]=new Preset("haunted",127,64-64,  43,   0, 108,   0,  48,  25, 10,14,    4,13, 4,  1,  3, 2);
        presets[14]=new Preset("Baroque",127,64-64,   0,   0,   0,  64,  24,   0, 14,17,    7, 0,32,  3,  4, 2);
        presets[15]=new Preset("Horror",127,64+63, 127,   0,  29, 127, 127,  33, 13,32,    2, 7, 7,  7,  3, 1);
        presets[16]=new Preset("Horror Pad", 47,64+ 0, 127,   0,  62,  86,  54,  32, 17,11,    4,12, 5,  0,  4, 1);
        ...
        presets[21]=new Preset("Nice noise", 38,64+0 ,   0,   3,   0,  97,   0,  74, 28, 5,    0,20, 5,  0,  2, 3);
        ...
        presets[26]=new Preset("SF",127,64-64,  28,   2,  36,   0, 127,  34,  2, 7,    0, 0,28,  3,  5, 1);
